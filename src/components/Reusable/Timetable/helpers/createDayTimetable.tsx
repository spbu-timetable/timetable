	...
	for (let i = 0; i < event.main.length; i++) {
		items.push(
			<div key={i} className={style.cell_item}>
				<Typography color="textPrimary">{event.main[i]}</Typography>
				<br />
				<Typography color="textPrimary">{event.extra[i]}</Typography>
				<br />
				<Typography color="textPrimary"> {event.address![i]}</Typography>
				<br />
				<Typography color="textPrimary"> {event.groups![i]}</Typography>
			</div>
		);
	}
	return items;
	...
	for (let i = 0; i < matrix.length; i++) {
		...
		row.push(
			<td key={i} className={style.td}>
				<Typography color="textPrimary">{intervals[i]}</Typography>
			</td>
		);
